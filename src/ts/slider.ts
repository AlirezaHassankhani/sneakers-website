...
let currentPicture: HTMLImageElement = document.querySelector(".current-picture")!;
...
function previousHandler() {
  let numberID: number = Number(currentPicture.dataset.id) - 1;
  if (numberID < 1) numberID = 4;

  let newID: string = String(numberID);
  changeImage(newID);
}

function nextHandler() {
  let numberID: number = Number(currentPicture.dataset.id) + 1;
  if (numberID > 4) numberID = 1;

  let newID: string = String(numberID);
  changeImage(newID);
}

function changeImage(newID: string) {
  ...
  let currentImg: HTMLImageElement = sliderNavigation?.querySelector(`[data-id="${newID}"]`)!;
